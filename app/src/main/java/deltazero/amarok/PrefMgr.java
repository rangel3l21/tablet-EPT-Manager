...
public final class PrefMgr {
    ...
    public static void init(Context context) {
        ...
        initialized = true;
        // If no custom blocked URLs yet, seed a default safe‑search list (social + games)
        if (mPrefs.getStringSet(BLOCKED_URLS, null) == null || mPrefs.getStringSet(BLOCKED_URLS, new HashSet<>()).isEmpty()) {
            Set<String> defaults = new HashSet<>();
            // Social media
            defaults.add("instagram.com");
            defaults.add("facebook.com");
            defaults.add("twitter.com");
            defaults.add("tiktok.com");
            defaults.add("snapchat.com");
            // Gaming sites
            defaults.add("roblox.com");
            defaults.add("poki.com");
            defaults.add("miniclip.com");
            defaults.add("kongregate.com");
            setBlockedUrls(defaults);
        }
    }
    ...
    // Firewall
    public static final String FIREWALL_ENABLED = "firewallEnabled";
    public static final String BLOCKED_URLS = "blockedUrls";
    ...
    public static void setHideApps(Set<String> pkgNames) {
        ...
        if (getSupabaseToken() != null) {
            deltazero.amarok.SyncManager.pushSettings(new deltazero.amarok.SyncManager.SyncResultCallback() {
                @Override
                public void onComplete(boolean success, String message) {
                    // Ignored silently for auto-sync
                }
            });
        }
    }
    ...
    // Supabase
    @Nullable
    public static String getSupabaseToken() {
        return mPrefs.getString(SUPABASE_TOKEN, null);
    }

    public static void setSupabaseToken(String token) {
        mPrefEditor.putString(SUPABASE_TOKEN, token);
        mPrefEditor.apply();
    }
    ...
    public static void setSupabaseUserId(String userId) {
        mPrefEditor.putString(SUPABASE_USER_ID, userId);
        mPrefEditor.apply();
    }

    @Nullable
    public static String getSupabaseEmail() {
        return mPrefs.getString(SUPABASE_EMAIL, null);
    }
    ...
    // Firewall
    public static boolean getFirewallEnabled() {
        return mPrefs.getBoolean(FIREWALL_ENABLED, false);
    }

    public static void setFirewallEnabled(boolean isEnabled) {
        mPrefEditor.putBoolean(FIREWALL_ENABLED, isEnabled);
        mPrefEditor.apply();
    }

    public static Set<String> getBlockedUrls() {
        return new HashSet<>(mPrefs.getStringSet(BLOCKED_URLS, new HashSet<>()));
    }

    public static void setBlockedUrls(Set<String> urls) {
        mPrefEditor.putStringSet(BLOCKED_URLS, urls);
        mPrefEditor.apply();
        // Auto-sync firewall settings if logged in
        if (getSupabaseToken() != null) {
            deltazero.amarok.SyncManager.pushSettings(new deltazero.amarok.SyncManager.SyncResultCallback() {
                @Override
                public void onComplete(boolean success, String message) {
